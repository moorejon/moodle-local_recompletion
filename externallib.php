...
class local_recompletion_external extends external_api {
    ...
    public static function get_course_completions($userids, $courseids, $sort = 'ASC', $limit = 0) {
        ...
        $sql = "SELECT comp.*
                  FROM (SELECT * FROM {course_completions} cc
    ...
    public static function update_core_completion($completions) {
        ...
        foreach ($params['completions'] as $data) {
            ...
            $event = \local_recompletion\event\externalapi_called::create(
                    array(
                            ...
                            'other' => [
                                    ...
                                    'newvalues' => $data,
                                    'oldvalues' => json_encode($corecompletion)
                            ]
                    )
            );
            $event->trigger();
            ...
            // Prevent overwrite of completions that were generated by site usage (completions created via web service have 0 timestarted)
            $threshold = $corecompletion->timecompleted - DAYSECS;
            if ($threshold > $data['timecompleted']) {
                throw new moodle_exception('newcorecompletionolderthanexisting');
            }
